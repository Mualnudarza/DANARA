...
export default function WalletsView({ data, balances, addWallet, updateWallet, deleteWallet }: {
  data: FinanceData;
  ...
  return (
    <div className="grid gap-5">
      <section className="rounded-xl border border-line bg-surface p-4 shadow-[0_1px_2px_0_rgb(0_0_0/0.05)]">
        ...
        <p className="mt-0.5 text-xs text-ink-500">Kelola dompet dan label rekening/e-wallet. Saldo dihitung dari riwayat transaksi.</p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {data.wallets.map((wallet) =>
              editingId === wallet.id ? (
                ...
                <WalletCard key={wallet.id} wallet={wallet} balance={balances[wallet.id] ?? 0} onEdit={() => setEditingId(wallet.id)} onDelete={() => deleteWallet(wallet.id)} />
              ),
            )}
          </div>
        )}
